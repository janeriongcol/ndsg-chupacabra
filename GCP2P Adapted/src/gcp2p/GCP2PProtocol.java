...
public class GCP2PProtocol implements Overlay, CDProtocol, EDProtocol{
	...
	*/
		...
		if(!left){
			for(int i = 0; i < numPeers; i++){
				...
				while (given < peerSpdAlloted[i]){
					...
					prot.sendMsg(new GCP2PMessage(GCP2PMessage.UPLOAD, node, peerList[i], essentialSize + speedLimit, peerSpdAlloted[i]), peerRTT[i]);
					given += toGive;
				}
				...
				if(uploaded > 4*videoSize && nodeTag!=0){
					...
					if(nodeTag == SuperPeerTag){
						...
						prot.sendMsg(new GCP2PMessage(GCP2PMessage.GOODBYE_AS_SP, node, connectedCDN, 171, prot.binID), cdnRTT);
					}
					else 
						// 171 = 168 + 3(3 bits lang since 6 values)
						prot.sendMsg(new GCP2PMessage(GCP2PMessage.GOODBYE_AS_NORMAL, node, connectedCDN, 171, prot.binID), cdnRTT);
					
					for(int j = 0; j < numPeers; j++){
						// 232 = 168 + 64 (int kasi)
						prot.sendMsg(new GCP2PMessage(GCP2PMessage.GOODBYE_AS_SOURCE, node, peerList[j], 232, peerSpdAlloted[j]), peerRTT[j]);
					}
	...
	public void processEvent( Node node, int pid, Object event ) {
		...
		GCP2PMessage omsg = (GCP2PMessage)event;
		...
		if (nodeTag == CDNTag){
			...
			*/
			...
			if (omsg.msgType == GCP2PMessage.GET_SUPERPEER){		//new peer requests for its bin's SP. aem.data is the binID
				...
				// 296 = 168 + IP(128)
				prot.sendMsg(new GCP2PMessage(GCP2PMessage.YOUR_SUPERPEER, node, omsg.sender, 296, sp), omsg.delay);
			}
			else if (omsg.msgType == GCP2PMessage.GET_SUPERPEER_2){
				// 296 = 168 + IP(128)
				p.sendMsg(new GCP2PMessage(GCP2PMessage.YOUR_SUPERPEER, node, omsg.sender, 296, superPeerList[omsg.data]), omsg.delay);		
			}
			else if (omsg.msgType == GCP2PMessage.DO_YOU_HAVE_THIS){			// this won't happen
				/**
				*	message received asking if the CDN has the video
				*/
				int i;
				int reply = GCP2PMessage.I_DONT_HAVE_IT;
				for(i = 0; i<videoList.length; i++){ // check if the requested video's id is in the list
					if(omsg.data == videoList[i]){
						reply = GCP2PMessage.I_HAVE_IT;
						break;
						}
				}
				Node temp = null;
				// 296 = 168 + IP(128) lagyan na lang rin kahit hindi nangyayari. haha
				p.sendMsg(new GCP2PMessage(reply, node, omsg.sender, 296, temp), omsg.delay);		
			}
			else if (omsg.msgType == GCP2PMessage.GET_MY_CLIENTS){		// a new SuperPeer requests for its clients. aem.data is the binID
				/**
				...
				*/
				...
				p.sendMsg(new GCP2PMessage(GCP2PMessage.YOUR_CLIENTS, node, omsg.sender, 0, temp, tempWatching, tempIndex, tempSize), omsg.delay);		
				...
				if(superPeerList[omsg.data]!=null){		// send this to notify the old SP that he is fired
					...
					p.sendMsg(new GCP2PMessage(GCP2PMessage.FIRED, node, superPeerList[omsg.data], 296, omsg.sender), prot.cdnRTT);
				}
				...
				for(int i = 0; i<5; i++){
					if(superPeerList[i] != null){
						...
						p.sendMsg(new GCP2PMessage(GCP2PMessage.UPDATE_SP, node, superPeerList[i], 424,superPeerList[omsg.data], omsg.sender), prot.cdnRTT);
					}
				}
				superPeerList[omsg.data] = omsg.sender;	// make the sender the SP				
			}
			else if (omsg.msgType == GCP2PMessage.GOODBYE_AS_SP){
				for (int i = 0; i < 6; i++){
					//remove as client
				...
				for(int i = 0; i < 6; i++){
					if(omsg.sender.equals(superPeerList[i])){
						...
						p.sendMsg(new GCP2PMessage(GCP2PMessage.YOUR_SUPERPEER, node, omsg.sender, 424, superPeerList[i], sp), omsg.delay);		
						break;
					}
			...
			else if (omsg.msgType == GCP2PMessage.GOODBYE_AS_NORMAL){
				for (int i = 0; i < 6; i++){
					//remove as client
					for(int j = 0; j < maxClients; j++){
						if(omsg.sender.equals(binList[i][j])){
			...
			if (omsg.msgType == GCP2PMessage.REQUEST_PEERS_FROM_THIS_BIN ){	// a Peer requests a SP for peers. aem.data0 - categoryID. aem.data - videoID
					GCP2PProtocol prot = (GCP2PProtocol)omsg.sender.getProtocol(pid);
					...
					*/
					...
					for (int i = 0; i < numClients; i++){
						...
						if (clientWatching[temp[i]] == omsg.data){
							...
							if(num == 6){
								...
								sendMsg(new GCP2PMessage(GCP2PMessage.INCOMPLETE_PEER_LIST, node, omsg.sender, 936, tempList), omsg.delay);
								num = 0;
								// TODO
					...
					sendMsg(new GCP2PMessage(GCP2PMessage.COMPLETE_PEER_LIST, node, omsg.sender, 168+tempList.size()*128, tempList), omsg.delay);
				...
				else if (omsg.msgType == GCP2PMessage.REQUEST_PEERS_FROM_OTHER_BINS){	// the peers returned in REQUEST_PEERS_FROM_THIS_BIN is empty
					for(int i= 0; i<5; i++){							// send requests to other SP and define the new peer as the sender. This will make the other SP
						...
						if(otherSP[i] != null)
							...
							sendMsg(new GCP2PMessage(GCP2PMessage.ANOTHER_BIN_PEER_REQUEST, omsg.sender, otherSP[i], 0, omsg.data0,omsg.data), CommonState.r.nextInt(100));
						else 
							...
							sendMsg(new GCP2PMessage(GCP2PMessage.OTHER_BIN_NUMBER_OF_PEERS, node, omsg.sender, 0, 0), omsg.delay);
					}
				}
				else if (omsg.msgType == GCP2PMessage.ANOTHER_BIN_PEER_REQUEST){
					...
					sendMsg(new GCP2PMessage(GCP2PMessage.OTHER_BIN_NUMBER_OF_PEERS, node, omsg.sender, 232, num), CommonState.r.nextInt(980)+20);	
					
				}
				else if (omsg.msgType == GCP2PMessage.UPDATE_SP){
					for(int i = 0; i<5; i++){
				...
				else if (omsg.msgType == GCP2PMessage.FIRED){									// the peer is not a SP anymore
					nodeTag = 2;
				}
		...
		// Node is a Regular Peer
			if(omsg.msgType == GCP2PMessage.UPLOAD){					// a chunk is delivered. aem.data is the chunk
				...
				if(streamedVideoSize>= videoSize){		// check if done streaming. if yes, send GOODBYE messages
					for(int i = 0; i< numSource; i++){
						...
						p.sendMsg(new GCP2PMessage(GCP2PMessage.GOODBYE, node, sourcePeerList[i], 168, 0), omsg.delay);		
					}
					doneStreaming = true;
				}
			}
			else if (omsg.msgType == GCP2PMessage.YOUR_PEERS){			// The peerList was sent by the SP
				int i = 0;
					if(omsg.data == 0){						// if the list is empty, request from other bins
						if(SPreply == 0){					// if SPreply = 0 means that it was sent from the peer's SP
							// TODO size of the message
							p.sendMsg(new GCP2PMessage(GCP2PMessage.REQUEST_PEERS_FROM_OTHER_BINS, node, omsg.sender, 0, categoryID, videoID), 0);				
						}
						...
						if(SPreply == 0){					// if SPreply = 0 then the peers is from it's bin
							while(usedDownloadSpd < downloadSpd && i < omsg.data){
								if(omsg.nodeList[i]!= null) {
									// TODO size of the message
									p.sendMsg(new GCP2PMessage(GCP2PMessage.CONNECT, node, omsg.nodeList[i], 0, downloadSpd - usedDownloadSpd), 0);		
					...
					if(SPreply == maxBins){						// means that all the bins have sent its peers
						...
						while(usedDownloadSpd < downloadSpd && i < highestStreamingSameVid){	//send CONNECT messages to the bin with the highest number of peers
							if(candidatePeers[i]!=(null))
								// TODO size of the message
								p.sendMsg(new GCP2PMessage(GCP2PMessage.CONNECT, node, candidatePeers[i], 0, downloadSpd - usedDownloadSpd), 0);		
								numConnectionsAttempted++;
								i++;
							}
						if(highestStreamingSameVid == 0){
							// TODO size of the message
							p.sendMsg(new GCP2PMessage(GCP2PMessage.CONNECT, node, connectedCDN, 0, downloadSpd - usedDownloadSpd), cdnRTT);		
							numConnectionsAttempted++;				
						}
						//i++;
						SPreply = 0;
			...
			else if (omsg.msgType == GCP2PMessage.INCOMPLETE_PEER_LIST){
				while (!omsg.linkedNodeList.isEmpty()){
					peerPool.add(omsg.linkedNodeList.remove());
				}
			}
			else if (omsg.msgType == GCP2PMessage.COMPLETE_PEER_LIST){
				...
				if (peerPool.isEmpty() && spSent == 5){
					...
					sendMsg(new GCP2PMessage(GCP2PMessage.REQUEST_PEERS_FROM_OTHER_BINS, node, omsg.sender, 232, categoryID, videoID), CommonState.r.nextInt(480)+20);				
				}
				int iterate = list.length;
				
				for (int i = 0; i < iterate && usedDownloadSpd < downloadSpd; i++){
					if(list[i]!=null){
						p.sendMsg(new GCP2PMessage(GCP2PMessage.CONNECT, node, list[i], 232, downloadSpd - usedDownloadSpd), RTTs[i]);		
						numConnectionsAttempted++;
			...
			else if (omsg.msgType == GCP2PMessage.OTHER_BIN_NUMBER_OF_PEERS){
				spSent++;
				...
				if(spSent == 5 ){
					if(potentialSource!= null){
						//232 = 168 + 32 bits for category + 32 bits for videoID
						sendMsg(new GCP2PMessage(GCP2PMessage.REQUEST_PEERS_FROM_THIS_BIN, node, potentialSource, 232, categoryID, videoID), 0);
					}
					else{
			...
			else if (omsg.msgType == GCP2PMessage.UPLOAD_SPEED_THAT_CAN_BE_GIVEN){	// reply to the CONNECT request. aem.data is the maximum upload spd that can be given
					...
					if (spdAvail > 0){									// check if the available download spd is not yet maxed
						...
						p.sendMsg(new GCP2PMessage(GCP2PMessage.REJECT_SPEED, node, omsg.sender, 232, omsg.data), omsg.delay);		
					}
					
					
			}
			else if (omsg.msgType == GCP2PMessage.ACCEPT_SPEED){						// reply to the proposed upload spd. aem.date0 is the proposed upload spd. aem.data is the acceoted sod
			...
			else if (omsg.msgType == GCP2PMessage.REJECT_SPEED){						// if the upload spd is rejected, remove the reserved uploadSpd in the uploadSpdBuffer
				uploadSpdBuffer = uploadSpdBuffer - omsg.data;
				startedStreaming = true;
			}
			else if (omsg.msgType == GCP2PMessage.CONNECT){								// a peer is requesting for upload Spd
				...
				if(spdAvail>0){		
					...
					// 232 = 168 + 64bits for speed
					p.sendMsg(new GCP2PMessage(GCP2PMessage.UPLOAD_SPEED_THAT_CAN_BE_GIVEN, node, omsg.sender, 232, spdAvail), omsg.delay);		
					...
					p.sendMsg(new GCP2PMessage(GCP2PMessage.REJECT, node, omsg.sender, 168, 0), omsg.delay);		
				}
				//startedStreaming = true;
				//if(nodeTag != 0)
			...
			else if (omsg.msgType == GCP2PMessage.REJECT){
					//hindi ko pa alam ano mangyayari
				numConnectionsRejected++;
			}
			else if (omsg.msgType == GCP2PMessage.YOUR_CLIENTS){							// the CDN sent your clients
				nodeTag = 1;
				...
				// 171 = 168 + 3bits
				p.sendMsg(new GCP2PMessage(GCP2PMessage.GET_SUPERPEER, node, prot.connectedCDN, 171, binID), cdnRTT);		
			}
			else if (omsg.msgType == GCP2PMessage.YOUR_SUPERPEER){						// not gonna happen
				if(omsg.superPeer!=null){
					// 232 = 168 + 32 bits for categoryID + 32bits for videoID
					p.sendMsg(new GCP2PMessage(GCP2PMessage.REQUEST_PEERS_FROM_THIS_BIN, node, omsg.superPeer, 232, categoryID, videoID), CommonState.r.nextInt(480)+20);		
					...
					// 171 = 168 + 3bits
					p.sendMsg(new GCP2PMessage(GCP2PMessage.GET_MY_CLIENTS, node, omsg.sender, 171, binID), cdnRTT);		
				}
			}
			
			else if (omsg.msgType == GCP2PMessage.GOODBYE){
			...
			else if (omsg.msgType == GCP2PMessage.GOODBYE_AS_SOURCE){
	...
	public void sendMsg(GCP2PMessage orangeMsg, int delay)
	{
	...
	public void start(Node node){
		GCP2PProtocol prot = (GCP2PProtocol) node.getProtocol(pid);
		// TODO size of the message
		prot.sendMsg(new GCP2PMessage(GCP2PMessage.GET_SUPERPEER, node, prot.connectedCDN, 168, binID), cdnRTT);
